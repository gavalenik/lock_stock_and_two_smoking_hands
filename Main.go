...
var (
    ...
    index_time = current_time.Add(6*time.Hour)
    token = get_token_from_file("token")
    current_USD, current_EUR, current_RUB float64 = 0,0,0
    sp500, us30 float64
    stock_count int = 10
    stocks [stock_count]string
    stock_prices [stock_count]float64
)
...
func get_index_value(info string) (float64, int) { //getting index value from string with 6 indices
...
func getting_sp500_nasdaq() { //getting body with 6 indices
...
func balance_difference(client *sdk.RestClient) {
    ...
    if time.Now().After(balance_time) == true {
        ...
        differnce_USD := current_USD - old_USD
        differnce_EUR := current_EUR - old_EUR
        differnce_RUB := current_RUB - old_RUB

        var msg = "Hey Bro! There's a difference between latest and current balance\nUSD: "+strconv.FormatFloat(differnce_USD,'f',2,64)+"\nEUR: "+strconv.FormatFloat(differnce_EUR,'f',2,64)+"\nRUB: "+strconv.FormatFloat(differnce_RUB,'f',2,64)+"\n"
        log.Println(msg)
        telegram (msg)
...
/*
func stock_info_request(client *sdk.RestClient, stocks [stock_count]string) {
...
func get_stock_info(client *sdk.RestClient) { //read list of stocks from file
    var i int = 0
...
func get_stock_orderbook(client *sdk.RestClient, stocks [stock_count]string) { //getting stock price
    var deep int = 10
...
//loop for {
    getting_sp500_nasdaq() //once per 6 hours because of limit for requests
    balance_difference(session) //once per 24 hours
    get_stock_orderbook(session, stocks) //getting stock price
//loop }
}
